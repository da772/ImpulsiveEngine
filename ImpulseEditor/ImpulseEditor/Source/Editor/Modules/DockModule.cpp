...
namespace Editor {
	...
	void DockModule::Create(const std::string& name, bool* is_open, uint32_t flags)
	{
		...
		if (opt_fullscreen)
		{
			...
			ImGui::SetNextWindowPos({ viewport->Pos.x , viewport->Pos.y + 30.f });
			ImGui::SetNextWindowSize({ viewport->Size.x, viewport->Size.y-60.f});
			ImGui::SetNextWindowViewport(viewport->ID);
			ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
			ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
			window_flags |= ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove;
