...
namespace GEngine {
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			/* FIX THIS RETARD                                                      */
			/************************************************************************/
			std::string fp;// = FileSystem::FilePath(filepath);
			m_OutputStream.open(fp);
	...
	public:
		InstrumentationTimer(const char* name)
			: m_Name(name), m_Stopped(false), fl()
		{
			m_StartTimepoint = std::chrono::steady_clock::now();
		}

		InstrumentationTimer(const char* name, float* time)
			: m_Name(name), m_Stopped(false), fl(time)
		{
			m_StartTimepoint = std::chrono::steady_clock::now();
		}
		...
		void Stop()
		{
			...
			float elapsedTime_float = float(elapsedTime.count() / 1000.f);
			if (fl!= nullptr) *fl = elapsedTime_float;
...
#define GE_PROFILE_TIMER(name, ptr) ::GEngine::InstrumentationTimer timer##__LINE__(name, ptr);
