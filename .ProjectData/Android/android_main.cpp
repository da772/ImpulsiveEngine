...
static int engine_init_display(struct engine* engine) {
	...
	* ANativeWindow buffers to match, using EGL_NATIVE_VISUAL_ID. */
	...
	//GEngine::Ref<GEngine::FileData> data = GEngine::Utility::FileDataFromPath(GEngine::Utility::FilePathFromRoot("Res/shaders/TextureShader.glsl"));
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
	...
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			LOGW("TOUCH %lu at %d: %f,%f", (unsigned long)id, action, x, y);
			GEngine::Mobile_Input_Callback::Touched(id, action, x, y, pressure);
		}
...
void android_main(struct android_app* state) {
	...
	while (1) {
		...
		if (checkResize) {
			...
			uint32_t w = ANativeWindow_getWidth(_engine->app->window);
			uint32_t h = ANativeWindow_getHeight(_engine->app->window);
			if (w != _engine->width && h != _engine->height) {
				checkResize = false;
				_engine->width = w;
				_engine->height = h;
				LOGW("RESIZED? %d, %d", w, h);
				App_Resize(_engine->width, _engine->height);
			}
