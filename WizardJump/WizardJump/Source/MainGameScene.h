...
public:
	...
	inline void OnBegin() override
	{
       ...
       // long l2 = lc->AddCircleLight({3,3}, 50, {1,1}, {1,1,1,.5f});
		...
		Ref<DialogFrame> dialog = CreateGameObject<DialogFrame>(Vector3f(0, .5f, 5), 15.f, "Wizard", "Content/Textures/wiz10_face.png", Vector4f(1, 1, 1, 1), "This is a big mountain. I wonder how I got up here. Maybe I should search that tower for clues so I can get out of here! \n(Tap here to continue...)", true, false);//aisdjiasjd aisdji asid jasid jaisdj aisdj ai aisdjiasjd->"));
	
		
		
		dialog->SetOnDialogComplete([this]() {
			
			Ref<DialogFrame> dialog = CreateGameObject<DialogFrame>(Vector3f(0, .5f, 5), 15.f, "Wizard", "Content/Textures/wiz10_face.png", Vector4f(1, 1, 1, 1), "Press and hold on the right side of the screen to move to the right!", false, false);//aisdjiasjd aisdji asid jasid jaisdj aisdj ai aisdjiasjd->"));
			...
			characterEntity->m_characterComponent->SetInputFilterFunction([this, dialog](const GEngine::FTouchInfo& touch) {
				...
				if ((touch.state == 0 || touch.state == 1) && touch.x <= (float)Application::GetWidth() / 2.f) {
					...
					if (characterEntity->m_characterComponent->bodyComp->GetVelocity().x > 2.f || characterEntity->GetEntityPosition().x > -8.f) {
						...
						characterEntity->m_characterComponent->bEnableInput = false;
						Ref<DialogFrame> _dialog = CreateGameObject<DialogFrame>(Vector3f(0, .5f, 5), 15.f, "Wizard", "Content/Textures/wiz10_face.png", Vector4f(1, 1, 1, 1),"Press and hold on the left side of the screen to move to the left!", false, false);//aisdjiasjd aisdji asid jasid jaisdj aisdj ai aisdjiasjd->"));
						...
						characterEntity->m_characterComponent->SetInputFilterFunction([this, _dialog](const GEngine::FTouchInfo& touch) {
							...
							if ( (touch.state == 0 || touch.state == 1) && touch.x >= (float)Application::GetWidth() / 2.f) {
								...
								if (characterEntity->m_characterComponent->bodyComp->GetVelocity().x < -2.f || characterEntity->GetEntityPosition().x < -10.0f) {
									...
									characterEntity->m_characterComponent->bEnableInput = false;
									Ref<DialogFrame> __dialog = CreateGameObject<DialogFrame>(Vector3f(0, .5f, 5), 15.f, "Wizard", "Content/Textures/wiz10_face.png", Vector4f(1,1,1,1), "I should keep walking to the right. \n(Tap here to continue...)", false, true);//aisdjiasjd aisdji asid jasid jaisdj aisdj ai aisdjiasjd->"));
									AddEntity(__dialog);
