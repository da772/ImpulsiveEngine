...
void FogEntity::OnUpdate(Timestep timestep)
{
	...
	while (m_threadCount < 3) {
		...
		ThreadPool::AddJob([this]() {
			...
			while (cFrame != frame - 1) {
				if (Time::GetEpochTimeMS() - ct > 10) {
					if (m_ending) return;
					std::lock_guard<std::mutex> guard(m_mutex);
					cFrame = m_frameProcessed;
					ct = Time::GetEpochTimeMS();
				}
			}

			{
				if (m_ending) if (m_ending) return;
				std::lock_guard<std::mutex> guard(m_mutex);
				ct = m_frameProccesedTime;
			}

			while (Time::GetEpochTimeMS() - ct < m_speed) {
				if (m_ending) return;
				std::lock_guard<std::mutex> guard(m_mutex);
				ct = ct = m_frameProccesedTime;
				continue;
			}
			...
			ThreadPool::AddMainThreadFunction([this, frame, noiseData]() {
				...
				if (!m_init) {
					...
					m_spriteId = m_spriteComponent->CreateQuad({ -4.75f,-3.5f,15 }, 0, { 12.5f,7.5f, 1 }, { 250.f / 255.f, 235.f / 255.f, 202.f / 255.f, m_alpha }, m_texture, { 1.f,1.f });
					m_spriteId = m_spriteComponent->CreateQuad({ 2.f,-3.5f,-1 }, 0, { -1.f,7.5f, 1 }, { 250.f / 255.f, 235.f / 255.f, 202.f / 255.f, m_alpha }, m_texture, { 12.5f,1.f });
					m_spriteId = m_spriteComponent->CreateQuad({ -4.75f,-17.25f,-1 }, 0, { 13.f,-20.f, 1 }, { 250.f / 255.f, 235.f / 255.f, 202.f / 255.f, m_alpha }, m_texture, { 1.f,1.f });
